...
public class Carnaval implements Serializable {
    ...
    public List<Carta> getCartas(List<Integer> indices) {
    ...
    public boolean agarroCartasSalvadasCarnaval(int valorCarta, List<Integer> cartasElegidas) {
    ...
    public boolean faltaAgarrarCartas(Carta carta, List<Integer> cartasElegidas) {
        ...
        if (cartasElegidas == null) {
            ...
            cantidadCartasElegidas = cartasElegidas.size();
        }
        int contador = 0;

        for (Carta cartaCarnaval : this.cartas) {
            if (!cartaCarnaval.equals(carta) && !salvadas.contains(cartaCarnaval)
                    && (carta.equalsColor(cartaCarnaval) || cartaCarnaval.getValor() <= carta.getValor())) {
                contador++;
            }
        }
